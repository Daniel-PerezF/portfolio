...
export function ProjectDetails() {
  ...
  return (
    <div className="min-h-screen bg-inherit">
      ...
      <div className="text-white text-[0.92rem] mx-4 lg:w-full lg:flex lg:justify-center lg:text-lg">
        ...
        <div className="w-full lg:w-1/2">
          {project.features.map((feature, index) => (
            <div key={index} className="py-1 text-white">
              <button
                className="w-full text-left p-2 bg-gray-800 text-white hover:outline transition ease-in-out"
                onClick={() =>
              ...
              {openAccordionIndex === index && (
                <div className="p-2 text-[0.92rem]">{feature.desc}</div>
              )}
            </div>
          ))}
        </div>
